...
type Entry = { key: string; label: string; sublabel?: string };

function getEntries(teeTime: TeeTime): Entry[] {
  if (teeTime.format === "scramble" && teeTime.teams.length > 0) {
    return teeTime.teams.map((t) => ({ key: t.name, label: t.name, sublabel: t.players.join(" & ") }));
  }
  return teeTime.players.map((p) => ({ key: p, label: p }));
}
      ...
      <table className="min-w-full text-sm">
        <thead>
          <tr className="border-b border-zinc-200 bg-zinc-50">
            ...
            {entries.map((entry) => (
              <th key={entry.key} className="px-3 py-2 text-left font-medium text-zinc-500">
                <div>{entry.label}</div>
                {entry.sublabel && (
              ...
              {entries.map((entry) => (
                <td key={entry.key} className="px-1.5 py-1.5">
                  <input
                    ...
                    className="w-12 rounded-md border border-zinc-300 px-2 py-1 text-center"
                  />
                </td>
              ))}
            </tr>
